...
class TestToCsvdb:
    def test_from_sqlite(self, sample_sqlite, temp_dir):
        ...
        result = csvdb_python.py_to_csvdb(str(sample_sqlite), output=output)
    ...
    def test_force_overwrites(self, sample_sqlite, temp_dir):
        output = str(temp_dir / "out.csvdb")
        csvdb_python.py_to_csvdb(str(sample_sqlite), output=output)
        # Should succeed with force=True
        csvdb_python.py_to_csvdb(str(sample_sqlite), output=output, force=True)

    def test_no_force_fails_on_existing(self, sample_sqlite, temp_dir):
        output = str(temp_dir / "out.csvdb")
        csvdb_python.py_to_csvdb(str(sample_sqlite), output=output)
        with pytest.raises(RuntimeError, match="already exists"):
            csvdb_python.py_to_csvdb(str(sample_sqlite), output=output, force=False)

    def test_compress(self, sample_sqlite, temp_dir):
        output = str(temp_dir / "compressed.csvdb")
        csvdb_python.py_to_csvdb(str(sample_sqlite), output=output, compress=True)
...
class TestToCsvdbIncremental:
    def test_incremental_first_run(self, sample_sqlite, temp_dir):
        ...
        result = csvdb_python.py_to_csvdb_incremental(str(sample_sqlite), output=output)
...
class TestToSqlite:
    def test_from_csvdb(self, sample_csvdb):
        result = csvdb_python.py_to_sqlite(str(sample_csvdb), force=True)
        assert result.endswith(".sqlite")
        assert os.path.isfile(result)

    def test_tables_filter(self, sample_csvdb):
        result = csvdb_python.py_to_sqlite(str(sample_csvdb), force=True, tables=["users"])
        assert os.path.isfile(result)


class TestToDuckdb:
    def test_from_csvdb(self, sample_csvdb):
        result = csvdb_python.py_to_duckdb(str(sample_csvdb), force=True)
...
class TestToParquetdb:
    def test_from_csvdb(self, sample_csvdb, temp_dir):
        ...
        result = csvdb_python.py_to_parquetdb(str(sample_csvdb), output=output, force=True)
        assert result == output
        assert os.path.isdir(output)
...
class TestSqlQuery:
    def test_basic_query(self, sample_sqlite):
        rows = csvdb_python.sql_query(str(sample_sqlite), "SELECT * FROM users ORDER BY id")
        assert len(rows) == 3
    ...
    def test_query_with_where(self, sample_sqlite):
        rows = csvdb_python.sql_query(str(sample_sqlite), "SELECT name FROM users WHERE score > 90")
    ...
    def test_query_count(self, sample_sqlite):
        rows = csvdb_python.sql_query(str(sample_sqlite), "SELECT count(*) as cnt FROM users")
        assert len(rows) == 1
        assert rows[0]["cnt"] == "3"

    def test_query_csvdb(self, sample_csvdb):
        rows = csvdb_python.sql_query(str(sample_csvdb), "SELECT * FROM users ORDER BY id")
        assert len(rows) == 3
        assert rows[0]["name"] == "Alice"

    def test_null_values(self, sample_csvdb_with_nulls):
        rows = csvdb_python.sql_query(str(sample_csvdb_with_nulls), "SELECT * FROM data ORDER BY id")
    ...
    def test_rejects_non_select(self, sample_sqlite):
        with pytest.raises(RuntimeError, match="SELECT"):
            csvdb_python.sql_query(str(sample_sqlite), "DROP TABLE users")

    def test_with_cte(self, sample_sqlite):
        rows = csvdb_python.sql_query(
            str(sample_sqlite),
...
class TestChecksum:
    def test_returns_hex_hash(self, sample_csvdb):
        h = csvdb_python.checksum_db(str(sample_csvdb))
        assert len(h) == 64
        assert all(c in "0123456789abcdef" for c in h)

    def test_same_data_same_hash(self, sample_csvdb):
        h1 = csvdb_python.checksum_db(str(sample_csvdb))
        h2 = csvdb_python.checksum_db(str(sample_csvdb))
        assert h1 == h2

    def test_consistency_across_formats(self, sample_csvdb):
        csvdb_hash = csvdb_python.checksum_db(str(sample_csvdb))
        sqlite_path = csvdb_python.py_to_sqlite(str(sample_csvdb), force=True)
        sqlite_hash = csvdb_python.checksum_db(sqlite_path)
        assert csvdb_hash == sqlite_hash


class TestDiff:
    def test_identical(self, sample_csvdb):
        result = csvdb_python.diff_db(str(sample_csvdb), str(sample_csvdb))
    ...
    def test_different(self, temp_dir):
        ...
        result = csvdb_python.diff_db(str(dir1), str(dir2))
        assert result is True  # has differences


class TestValidate:
    def test_valid(self, sample_csvdb):
        result = csvdb_python.validate_db(str(sample_csvdb))
    ...
    def test_invalid_missing_schema(self, temp_dir):
        ...
        bad_dir.mkdir()
        result = csvdb_python.validate_db(str(bad_dir))
...
class TestInit:
    ...
    def test_init_detect_pk_disabled(self, raw_csv_dir):
        result = csvdb_python.init_csvdb(str(raw_csv_dir), detect_pk=False)
        table = result["tables"][0]
...
class TestErrorHandling:
    def test_bad_path(self):
        with pytest.raises(RuntimeError):
            csvdb_python.checksum_db("/nonexistent/path.csvdb")

    def test_bad_order_mode(self, sample_sqlite, temp_dir):
        with pytest.raises(RuntimeError, match="Unknown order mode"):
            csvdb_python.py_to_csvdb(str(sample_sqlite), output=str(temp_dir / "x.csvdb"), order="invalid")

    def test_bad_null_mode(self, sample_sqlite, temp_dir):
        with pytest.raises(RuntimeError, match="Unknown null mode"):
            csvdb_python.py_to_csvdb(str(sample_sqlite), output=str(temp_dir / "x.csvdb"), null_mode="invalid")
